...
Instead of "a sensor tripped → sound the alarm" (which a roaming cat or a
...
- **Cat-proof by design** — a single isolated trip barely moves the score; it
  takes sustained or multi-room movement to cross a cutpoint.
...
- Native `alarm_control_panel` with **Away / Home / Night / Vacation** modes,
  each watching its own configurable set of sensors (arm Home/Night and upstairs
  movement is ignored while you sleep).
...
5. Walk through the three setup steps (core settings → sensors per mode →
   outputs & response).
